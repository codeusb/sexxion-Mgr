...
const Publish:FC = ()=>{
  ...
  const getArticle = async () =>{
    ...
    //表单回显
    from.current!.setFieldsValue({ ...formValue, type: cover.type }) //!.认为form.curren不为空
    const imageList = cover.images.map((url:any) => ({ url })) //es6对象简写
        ...
        >
          ...
          >
            <Select placeholder="请选择文章频道" style={{ width: 400 }}>
              {channel?.map(
                (item:any)=><Option key={item.id} value={item.id}>{item.name}
                </Option>)}    
            </Select>
          </Form.Item>
